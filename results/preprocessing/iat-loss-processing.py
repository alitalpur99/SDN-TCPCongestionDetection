...
for i in range(len(diff)):
	##From graph check how much the gap (diff) value should be
	if values[i]>=0.12 and diff[i]>=700:
		# print indexes[i+1]
...
"""Saving the results to a File name iat-results"""

open("iat-loss-results","w").close()
with open("iat-loss-results","w") as f:
...
lines = []
with open('iat-loss-results') as infile:
    for line in infile:
        for src, target in replacements.iteritems():
            line = line.replace(src, target)
        lines.append(line)
print lines
with open('iat-loss-results', 'w') as outfile:
    for line in lines:
        outfile.write(line)
